  ...
  (let [{:keys [total leftover]} (play-out-fight world cell my-ship their-ship)]
    ...
    (if (<= 0 leftover)
      ...
      (let [my-closest-base (get-closest-cell world cell (-> world :my-player :dropoffs))
        ...
        (if (and (< (:distance my-closest-base) (:distance their-closest-base))
                 (< (:distance my-closest-base) 5))
          (+ total (* 0.5 leftover))
          (if (or (= (:distance my-closest-base) (:distance their-closest-base))
                  (> (:distance their-closest-base) 5))
            total
            (- total (* 0.5 leftover))))))))
